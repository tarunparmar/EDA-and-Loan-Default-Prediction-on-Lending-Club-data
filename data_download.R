...
for(i in seq_along(urls)){
  ...
  loan_df[[i]] <- read.csv(datafilename,skip = 1, stringsAsFactors = FALSE)
}
...
loan_df <- rbind.fill(loan_df)

saveRDS(loan_df,paste(filepath,"loan_data.Rds",sep = ""))
...
declined_loan_df$AR<-"Declined" # Create Variable to mark as "Declined" or "Issued"
declined_loan_df$grade<-"Z" # create grade "z" for rejected loans - Match column of accepted loan
declined_loan_df$id<-10000000 # create a common id for rejected loans - Match column of accepted loan
